...
public class P41_MedianOfThree {
		...
		for (int row = 0; row < count; row++) {
			...
			for (int col = 0; col < numCols; col++) {
				if (values[row][col] > max) { 
					max = values[row][col];
					if (col < 1) min = max; /* only sets min equal to max once for every row */
				}
				if (values[row][col] < min) min = values[row][col];
			}
			
			// sets the median value to whatever the min and max values aren't
			for (int col = 0; col < numCols; col++) {

				if (values[row][col] != min && values[row][col] != max) median = values[row][col];
			}
